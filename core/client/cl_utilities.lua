...
Draw3dTextHud = function(text, color, coordX, coordY, scaleX, scaleY) -- Draw Text Hud Function
    SetTextFont(4)
    ...
    BeginTextCommandDisplayText("STRING")
    AddTextComponentSubstringPlayerName(text)
    EndTextCommandDisplayText(coordX, coordY)
end,
    ...
    InsertModel = function(model)
        ...
        if HasModelLoaded(model) then
            return {inserted = true, model = model}
        else
            return {inserted = false, model = model}
        end
    end
}
